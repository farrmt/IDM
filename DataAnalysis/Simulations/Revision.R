...
for(i in 1:iter){
  ...
  Nst <- function(){
    Nst <- rep(1, W*W)
    for(i in 1:data5$B){
      Nst[data5$coverage[i]] <- Nst[data5$coverage[i]] + data5$x[i]
    }
    return(Nst)
  }

  inits5 <- function(){list(N = Nst(), sigma = runif(1, sigma-0.05, sigma+0.05), 
  ...
  S[[2]] <- jagsUI(data1, inits1, params, "PO2.txt", n.thin=nt, 
                   n.chains=nc, n.burnin=nb, n.iter=ni, n.adapt=na, parallel = TRUE)
  
  S[[3]] <- jagsUI(data1, inits1, params, "PO3.txt", n.thin=nt, 
                   n.chains=nc, n.burnin=nb, n.iter=ni, n.adapt=na, parallel = TRUE)
  
  S[[4]] <- jagsUI(data5, inits5, params, "ISDM.txt", n.thin=nt, 
                   n.chains=nc, n.burnin=nb, n.iter=ni, n.adapt=na, parallel = TRUE)
  
  S[[5]] <- jagsUI(data5, inits5, params, "ISDM2.txt", n.thin=nt, 
                   n.chains=nc, n.burnin=nb, n.iter=ni, n.adapt=na, parallel = TRUE)
  
  S[[6]] <- jagsUI(data5, inits5, params, "ISDM3.txt", n.thin=nt, 
                   n.chains=nc, n.burnin=nb, n.iter=ni, n.adapt=na, parallel = TRUE)
  ...
  for(sc in 2:7){
    ...
    tryCatch({Out[i,sc+6,1] <- S[[sc-1]]$Rhat$Ntot}, error = function(e){})
    tryCatch({Out[i,sc+6,2] <- S[[sc-1]]$Rhat$lambda0}, error = function(e){})
    tryCatch({Out[i,sc+6,3] <- S[[sc-1]]$Rhat$beta1}, error = function(e){})
    tryCatch({Out[i,sc+6,4] <- S[[sc-1]]$Rhat$sigma}, error = function(e){})
    tryCatch({Out[i,sc+6,5] <- S[[sc-1]]$Rhat$p0}, error = function(e){})
    tryCatch({Out[i,sc+6,6] <- S[[sc-1]]$Rhat$alpha1}, error = function(e){})
  }
