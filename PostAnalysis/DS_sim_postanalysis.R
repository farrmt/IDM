...
filenames <- list.files(pattern = "DS2018")
...
for(i in 2:length(filenames)){
  ...
  for(j in 1:length(output$Out[,1,1])){
    if(max(output$Out[j,4:5,], na.rm = TRUE) < 1.1)
...
hist(Out[,1,2]-Out[,2,2], main = "DS", xlab = "Intercept Bias") #DS vs Truth
hist(Out[,1,2]-Out[,3,2], main = "DS Alt", xlab = "Intercept Bias") #DSalt vs Truth

#Beta1
mean(abs(Out[,1,3]-Out[,2,3])) #DS
mean(abs(Out[,1,3]-Out[,3,3])) #DSalt

hist(Out[,1,3]-Out[,2,3], main = "DS", xlab = "Effect Bias") #DS vs Truth
hist(Out[,1,3]-Out[,3,3], main = "DS Alt", xlab = "Effect Bias") #DSalt vs Truth

#Abundance
mean(abs(Out[,1,1]-Out[,2,1])) #DS
mean(abs(Out[,1,1]-Out[,3,1])) #DSalt

hist(Out[,1,1]-Out[,2,1], main = "DS", xlab = "Abundance Bias") #DS vs Truth
hist(Out[,1,1]-Out[,3,1], main = "DS Alt", xlab = "Abundance Bias") #DSalt vs Truth
